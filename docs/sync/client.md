...
Two advisory locks live under `~/.tim/sync-locks/` (directory mode `0700`). A lock file is mode `0600` and records pid, Linux `/proc/<pid>/stat` start time, and a random token. `flock` is not required. A lock is published by writing a complete record to a temporary file and `link(2)`-ing it onto the lock name, so the name never exists without a readable record. A holder is alive only when `kill(pid, 0)` succeeds and the start time still matches. Without `/proc` the start time is not recorded and `kill` alone counts: a reused pid then keeps the lock until the file is deleted by hand. An unreadable lock file counts as live until it is 60 s old. A dead or reused pid is stale: recovery renames the lock aside and deletes it only when the renamed record is still the dead holder. A live holder is never stolen.

The **mutation lock** (`mutation.lock`) is one lock for the sync directory. Push, pull, connect, mirror, repair, disconnect, and every config, state, or queue read-modify-write take it and hold it across the whole operation, including network waits. Atomic rename is crash safety only. It does not stop two processes from both reading the old file and both renaming a replacement. The lock is what stops that. Nested calls in the same async operation re-enter. A different process waits, for at most 30 s and never past the cycle deadline. A same-process caller that is not inside the operation fails with `SYNC_LOCK_BUSY` at once instead of deadlocking. `tim sync push` and `tim sync pull` exit 2 when the lock is not acquired.
