...
Read complete issue bodies and comments before implementing. Use temporary UTF-8 body files with `--body-file` for multiline GitHub writes. Close a ticket only after integration and verification, and record the implementation commit and checks. Parent issues are not automatically closed or edited. Pull requests are not a separate triage input.
